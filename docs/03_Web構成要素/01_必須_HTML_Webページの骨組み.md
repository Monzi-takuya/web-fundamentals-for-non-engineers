...
HTMLにおける**アクセシビリティ**（すべてのユーザーが利用できるWebサイトの実現）は非常に重要なトピックです。障害の有無に関わらず、すべてのユーザーがWebサイトを利用できるようにする必要があります。

**アクセシビリティの実装方法については、別の専用ドキュメント『[06_応用_HTMLアクセシビリティの実装と要件定義.md](./06_応用_HTMLアクセシビリティの実装と要件定義.md)』で詳しく解説しています。**

**この章では、基本的なアクセシビリティ要素のみを紹介します：**
...
| 要素 | アクセシビリティ効果 | 支援技術での効果 |
|------|---------------------|------------------|
| `<button type="submit">` | キーボード操作可能、明確な役割 | スクリーンリーダーで「ボタン」として認識、Enterキーで実行可能 |
| `alt="売上推移グラフ"` | 画像内容の文字説明 | 視覚障害者に画像の内容を音声で伝達 |
| `<label for="name">` | 入力項目の明確な説明 | クリックで入力欄にフォーカス、スクリーンリーダーで関連付けを読み上げ |
| `id="name"` | label要素との関連付け | for属性とIDで入力欄と説明を結びつけ |
| `required` | 必須項目の明示 | スクリーンリーダーで「必須」と読み上げ、ブラウザの標準バリデーション |

**必要最低限のアクセシビリティ対応：**

```
🔴 必須対応（法的・技術的要件）
✅ 画像のalt属性設定（装飾画像は alt=""）
✅ フォーム要素とlabel要素の関連付け（for属性とID）
✅ 適切なボタン・リンクの使用（div clickではなくbutton、a要素）
✅ 見出しの論理的階層（h1→h2→h3の順序）

🟡 推奨対応（ユーザビリティ向上）
✅ required属性での必須項目明示
✅ リンクテキストの内容明記（「こちら」ではなく「求人詳細を見る」）
✅ 適切なコントラスト比（文字と背景の色彩差）
✅ キーボードでのフォーカス表示
```

**より詳細な内容については専用ドキュメントをご覧ください：**
- WCAG 2.1準拠のための具体的実装方法
- aria属性の活用と動的機能のアクセシビリティ
...
2. アクセシビリティ要件
   ✅ 基本要件は『06_応用_HTMLアクセシビリティの実装と要件定義.md』を参照
   ✅ WCAG 2.1 AA レベル準拠
   ✅ キーボード操作対応、適切なalt属性設定
...
・セマンティックHTML（fieldset/legend、label要素）の使用
・required属性で必須項目を明示  
・送信ボタンはbutton要素（type="submit"）を使用
・アクセシビリティ詳細は『06_応用_HTMLアクセシビリティの実装と要件定義.md』を参照」
```
...
□ リンクテキストが内容を的確に表している
※ 詳細は『06_応用_HTMLアクセシビリティの実装と要件定義.md』を参照
...
3. **Lighthouse タブ**: 総合評価
   ...
   - Accessibility: アクセシビリティスコア（詳細分析は専用ドキュメント参照）
...
- **フォーム要素**は適切なlabel、fieldset属性でアクセシビリティを確保
- **メタデータ**（title、meta、OGP、構造化データ）により検索エンジンとSNSでの表示を最適化
- **アクセシビリティ**の詳細については『[06_応用_HTMLアクセシビリティの実装と要件定義.md](./06_応用_HTMLアクセシビリティの実装と要件定義.md)』で体系的に学習

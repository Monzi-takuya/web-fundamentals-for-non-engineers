...
<details>
<summary><strong>🔧 サイト表示トラブルの診断フローと対処法</strong></summary>
...
flowchart TD
    ...
    B -->|はい| C["ブラウザでIPアドレス直接アクセス<br/>例: 142.250.196.110"]
    B -->|いいえ| D["基本的なネットワーク接続確認<br/>ping 8.8.8.8（IPアドレス直接）"]
    
    C -->|表示される| E["DNS問題<br/>nslookup ドメイン名で確認"]
    C -->|表示されない| F["サーバー問題<br/>開発者・運用チームに連絡"]
    
    D -->|応答あり| G["DNS名前解決の確認<br/>nslookup google.com"]
    D -->|応答なし| H["ネットワーク環境確認<br/>IT部門に連絡"]
    
    G -->|成功| I["対象サイトのDNS確認<br/>nslookup your-domain.com"]
    G -->|失敗| J["プロバイダのDNS確認<br/>8.8.8.8に変更して試す"]
    
    E --> K["DNS設定を確認・修正<br/>1-24時間で反映"]
    F --> L["サーバー再起動<br/>またはサービス復旧"]
    I --> M["サイト固有のDNS問題<br/>ドメイン管理者に連絡"]
    J --> N["プロバイダまたは<br/>ローカルDNS設定の問題"]
...
# 4. 直接IP確認（DNS問題の切り分け）
# IPアドレスがわかる場合のみ、ブラウザで直接アクセス

# 5. DNS切り替えテスト（プロバイダDNSの問題確認）
nslookup your-domain.com 8.8.8.8
```

**現在使用中のDNSサーバーの確認方法：**
```powershell
# Windows: 現在のネットワーク設定を確認
ipconfig /all
...
**8.8.8.8とは：**
GoogleのパブリックDNSサーバー。プロバイダのDNSサーバーに問題がある場合の切り分けに使用。これで解決すれば、プロバイダのDNS問題と判断できる。

**DNS問題の切り分けパターン：**

**パターン1: 全般的なDNS問題**
- `ping 8.8.8.8` → 成功（ネットワークは正常）
- `nslookup google.com` → 失敗
- **原因**: プロバイダのDNSサーバー全体の問題

**パターン2: 特定サイトのDNS問題**
- `ping 8.8.8.8` → 成功
- `nslookup google.com` → 成功
- `nslookup your-domain.com` → 失敗
- **原因**: 対象サイトのDNS設定問題

**パターン3: DNS切り替えで解決**
- デフォルトDNSで失敗 → `nslookup your-domain.com`
- 8.8.8.8で成功 → `nslookup your-domain.com 8.8.8.8`
- **原因**: プロバイダのDNSサーバーの部分的問題

**重要なポイント：**
`ping google.com` が成功しても、DNS設定に問題がある場合があります。これは：
- **DNSキャッシュ**: 以前の名前解決結果が残っている
- **部分的DNS問題**: google.comは解決できるが、他のドメインは失敗
- **DNS設定の不整合**: 一部のDNSサーバーのみ問題がある
